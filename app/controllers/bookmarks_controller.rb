...
#@emailId,@type,@firstName = emailId,type,firstName
    ...
    Mailer.deliver_message(
        {:recipients => emailId,
         :subject => "A new #{type} is submitted for #{assignment} ",
         :body => {
             :obj_name => assignment,
             :type => type,
  ...
  def emailOnBookMark(topicId)
    @team_member = Participant.find_all_by_topic_id(topicId)
    asstId = SignUpTopic.find_by_id(topicId).assignment_id
    assignment=Assignment.find_by_id(asstId).name
    puts "assignment name=#{assignment}"
    partialName="email_on_bookmark"
    @team_member.each do |team_Member|
     ...
     if(user.email_on_bookmark)
        email(user.email,"Bookmark",ApplicationHelper::get_user_first_name(user),assignment,partialName)
     end
    end
  end
