class CourseParticipant < Participant
  ...
  def self.export(csv, parent_id, options)
    ...
    tcsv = Array.new
      ...
      if (options["email_options"] == "true")
        tcsv.push(user.email_on_submission, user.email_on_review, user.email_on_review_of_review, user.email_on_bookmark)
      end
  ...
  def self.get_export_fields(options)
    ...
    if (options["email_options"] == "true")
      fields.push("email on submission", "email on review", "email on metareview", "email on bookmark")
    end
    if (options["handle"] == "true")
      fields.push("handle")
    end
